...
if($_SESSION['username'] != ''){
  ...
  $sql = "INSERT INTO rentreceiptproperty (rentReceiptID, propertyID, propertyAddress, referenceNumber,	rentReceiptDate,	paymentDate, employeeID, tenantName,	tenantMobile, tenantEmail, rentAmount, expenses, orderTotal, paymentTotal, paymentAmount, paymentType, paidDate, revenue, status)
  VALUES ('$rentReceiptID', '$propertyID', '$propertyAddress', '$referenceNumber', '$rentReceiptDate', '$paymentDate', '$employeeID', '$tenantName', '$tenantContact',
    '$tenantEmail', '$rentAmount', '$expense', '$orderTotal', '$paymentTotal', '$paymentAmount', '$paymentType', '$paidDate', '$revenue', '$status')";
    ...
    if ($con->query($sql)){
      ...
      $rentReceiptDate =  date_format($rentReceiptDate,"M-Y");

      $output .= '<b>Rent Month:</b> '.$rentReceiptDate.'<br>
      <b>Payment Status:</b>';
      ...
      if($tenantEmail != "" && $emailDocument == 1){
        ...
        if($mail->send()) {
          ...
          $sqlRemove = "DELETE FROM rentreceiptproperty WHERE rentReceiptID = '$rentReceiptID'";
          $con->query($sqlRemove);
        }
      }else{
        $response = "success";
