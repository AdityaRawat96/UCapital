...
if ($asset_type == "RE") {
    ...
    $sql = "UPDATE `business_company` SET `SUB_COMPANY_TYPE`='$startup_type',`COUNTRY`='$country',`CITY`='$city',`FOUNDATION_YEAR`=$foundation_year,`CURRENCY`='$default_currency',`COMPANY_VAL_TYPE`='$company_value_type',`COMPANY_VAL_MIN`=$company_value_min,`COMPANY_VAL_MAX`=$company_value_max,`INVESTMENT_REQ_TYPE`='$investment_required_value',`INVESTMENT_REQ_MIN`=$investment_required_min,`INVESTMENT_REQ_MAX`=$investment_required_max,`SECTOR`='$sector',`INDUSTRY`='$industry',`COMPANY_BUSINESS`='$company_business',`AREA_OF_ACTIVITY`='$area_of_activity',`SCALABILITY`='$scalability',`SCALABILITY_AREA`='$scalability_area',`MARKET_SHARE`=$market_share,`NUM_OF_EMPLOYEE_MIN`=$number_of_employees_min,`NUM_OF_EMPLOYEE_MAX`=$number_of_employees_max,`ACTUAL_REVENUE_TYPE`='$actual_revenue_type',`ACTUTAL_REVENUE_MIN`=$actual_revenue_min,`ACTUAL_REVENUE_MAX`=$actual_revenue_max,`EBIDTA_MARGIN`=$ebidta_margin,`FORECAST_REVENUE_Y1`=$for_rev_1,`FORECAST_REVENUE_Y2`=$for_rev_2,`FORECAST_REVENUE_Y3`=$for_rev_3,`FORECAST_EBITDA_Y1`=$for_ebd_1,`FORECAST_EBITDA_Y2`=$for_ebd_2,`FORECAST_EBITDA_Y3`=$for_ebd_3,`NUM_OF_INVESTMENT`=$number_of_investments,`PREF_INVESTMENT_MIN`=$preferred_investment_amount_min,`PREF_INVESTMENT_MAX`=$preferred_investment_amount_max,`WHO_I_AM`='$who_i_am',`LOOKING_FOR`='$looking_for',`WANT_TO_DO`='$what_i_want',`AUM`=$aum,`DESCRIPTION`='$description',`KEY_ELEMENTS`='$key_elements',`IMAGE`='$image' WHERE ID = '$id'";
}
